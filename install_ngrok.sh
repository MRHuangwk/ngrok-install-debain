...
read DOMAIN
install_dependency(){
	sudo apt-get install unzip perl cpio curl wget gcc
}
...
install_go(){
	...
	if [ $(getconf WORD_BIT) = '32' ] && [ $(getconf LONG_BIT) = '64' ];then
		...
		if [ ! -f $SELFPATH/go1.8.3.linux-386.tar.gz ];then
            wget https://dl.google.com/go/go1.8.3.linux-386.tar.gz
		fi
...
case "$num" in
	...
	;;
	[8] )
		/usr/local/ngrok/bin/ngrokd -domain=$DOMAIN -httpAddr=":80"
	;;
	[9] )
